...
CREATE INDEX IF NOT EXISTS idx_vb_assets_name_trgm
    ON vb_assets USING GIN (lower(name) gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_vb_findings_title_trgm
    ON vb_findings USING GIN (lower(title) gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_ck_risks_title_trgm
    ON ck_risks USING GIN (lower(title) gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_po_dsr_requester_trgm
    ON po_dsr USING GIN (lower(requester_name) gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_po_breaches_title_trgm
