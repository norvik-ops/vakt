...
// TestVaktprivacyRawSQLAgainstSchema (S135/GB-1) closes one of the five gaps
...
// concatenation anywhere in vaktprivacy (retention, DSR portal, Art. 17
// erasure, AVV/transfer records) would break at query time in production
// with no PR-time signal.
func TestVaktprivacyRawSQLAgainstSchema(t *testing.T) {
	...
	gen, err := sqlcheck.FromConsts("../../db/vaktprivacy.sql.go")
	require.NoError(t, err)
