// Package vaktprivacy provides business logic for DSGVO documentation: VVT, DPIA, AVV,
...
func (s *Service) CheckAVVExpiry(ctx context.Context) error {
	...
	if expired > 0 {
		log.Info().Int64("count", expired).Msg("vaktprivacy: marked AVVs as expired")
	}
	...
	for _, avv := range expiring {
		log.Info().
			...
			Msg("vaktprivacy: AVV review approaching")
	}
	return nil
}
...
func (s *Service) publishBreachCreated(ctx context.Context, b *Breach) {
	...
	if err != nil {
		log.Error().Err(err).Str("breach_id", b.ID).Msg("vaktprivacy: failed to marshal breach incident payload")
		return
	}
	if s.asynqClient != nil {
		...
		log.Info().
			Str("event_type", "vaktprivacy.breach.created").
			Str("breach_id", b.ID).
			Str("org_id", b.OrgID).
			Msg("vaktprivacy: breach created (Redis unavailable — incident not auto-created)")
	}
}
