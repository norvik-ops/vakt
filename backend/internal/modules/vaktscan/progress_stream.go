...
func PublishProgress(ctx context.Context, rdb *redis.Client, evt ProgressEvent) {
	...
	if err != nil {
		log.Warn().Err(err).Str("scan_id", evt.ScanID).Msg("vaktscan.progress: marshal failed")
		return
	}
	if err := rdb.Publish(ctx, progressChannel(evt.ScanID), payload).Err(); err != nil {
		log.Warn().Err(err).Str("scan_id", evt.ScanID).Msg("vaktscan.progress: publish failed")
	}
}
...
func (h *Handler) StreamScanProgress(c echo.Context) error {
	...
	tracer := otel.Tracer("vakt.vaktscan.scan.progress.stream")
