...
// CreateAsset handles POST /api/v1/vaktscan/assets.
func (h *Handler) CreateAsset(c echo.Context) error {
	...
	h.audit(c, "create", "vaktscan/asset", asset.ID, asset.Name)
	return c.JSON(http.StatusCreated, asset)
}

// ListAssets handles GET /api/v1/vaktscan/assets.
...
// GetAsset handles GET /api/v1/vaktscan/assets/:id.
...
func (h *Handler) UpdateAsset(c echo.Context) error {
	...
	h.audit(c, "update", "vaktscan/asset", assetID, asset.Name)
	return c.JSON(http.StatusOK, asset)
}

// DeleteAsset handles DELETE /api/v1/vaktscan/assets/:id.
...
// GetSLAConfig handles GET /api/v1/vaktscan/sla-config.
...
// UpdateSLAConfig handles PUT /api/v1/vaktscan/sla-config.
...
// ImportAssets handles POST /api/v1/vaktscan/assets/import (multipart/form-data, field "file").
...
// TriggerScan handles POST /api/v1/vaktscan/assets/:id/scans
func (h *Handler) TriggerScan(c echo.Context) error {
	...
	h.audit(c, "action", "vaktscan/scan", scan.ID, assetID)
	return c.JSON(http.StatusAccepted, scan)
}

// GetScan handles GET /api/v1/vaktscan/scans/:id
...
// ListFindings handles GET /api/v1/vaktscan/findings
...
// GetFinding handles GET /api/v1/vaktscan/findings/:id
...
// UpdateFinding handles PATCH /api/v1/vaktscan/findings/:id
func (h *Handler) UpdateFinding(c echo.Context) error {
	...
	if err != nil {
		...
		if err.Error() == "justification is required when setting status to accepted_risk" {
			return c.JSON(http.StatusUnprocessableEntity, map[string]string{"error": "justification is required when setting status to accepted_risk", "code": "VB_VALIDATION_ERROR"})
		}
		return c.JSON(http.StatusNotFound, map[string]string{"error": "finding not found", "code": "VB_FINDING_NOT_FOUND"})
	}
	h.audit(c, "update", "vaktscan/finding", findingID, finding.Title)
	return c.JSON(http.StatusOK, finding)
}
...
func (h *Handler) BulkUpdateFindings(c echo.Context) error {
	...
	h.audit(c, "update", "vaktscan/finding", "", "bulk")
	return c.JSON(http.StatusOK, map[string]int{"updated": n})
}

// ListSuppressions handles GET /api/v1/vaktscan/suppressions
...
// CreateSuppression handles POST /api/v1/vaktscan/suppressions
func (h *Handler) CreateSuppression(c echo.Context) error {
	...
	h.audit(c, "create", "vaktscan/suppression", rule.ID, cveID)
	return c.JSON(http.StatusCreated, rule)
}

// DeleteSuppression handles DELETE /api/v1/vaktscan/suppressions/:id
func (h *Handler) DeleteSuppression(c echo.Context) error {
	...
	h.audit(c, "delete", "vaktscan/suppression", suppressionID, "")
	return c.NoContent(http.StatusNoContent)
}

// ListScanSchedules handles GET /api/v1/vaktscan/assets/:id/schedules
...
// CreateScanSchedule handles POST /api/v1/vaktscan/assets/:id/schedules
func (h *Handler) CreateScanSchedule(c echo.Context) error {
	...
	h.audit(c, "create", "vaktscan/scan-schedule", schedule.ID, assetID)
	return c.JSON(http.StatusCreated, schedule)
}

// DeleteScanSchedule handles DELETE /api/v1/vaktscan/assets/:id/schedules/:schedule_id
func (h *Handler) DeleteScanSchedule(c echo.Context) error {
	...
	h.audit(c, "delete", "vaktscan/scan-schedule", scheduleID, "")
	return c.NoContent(http.StatusNoContent)
}

// GetRiskTrend handles GET /api/v1/vaktscan/reports/risk-trend
...
// GenerateReport handles POST /api/v1/vaktscan/reports
...
// DownloadReport handles GET /api/v1/vaktscan/reports/:id/download
...
// ListReports handles GET /api/v1/vaktscan/reports
...
// GetReport handles GET /api/v1/vaktscan/reports/:id
...
// TriggerSBOMScan handles POST /api/v1/vaktscan/assets/:id/sbom.
...
// GetAssetSBOM handles GET /api/v1/vaktscan/assets/:id/sbom.
...
// GetEOLDashboard handles GET /api/v1/vaktscan/sbom/eol.
