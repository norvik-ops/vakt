...
// GetNIS2Incident returns minimal incident data needed for NIS2 status/deadline checks.
func (r *Repository) GetNIS2Incident(ctx context.Context, orgID, incidentID string) (*NIS2IncidentRow, error) {
	var inc NIS2IncidentRow
	var (
	...
	if reportable.Valid {
		b := reportable.Bool
		inc.NIS2Reportable = &b
	}
	if stage.Valid {
		s := stage.String
		inc.NIS2ReportingStage = &s
	}
	inc.NIS2DetectedAt = ckTsToTimePtr(detectedAt)
...
func (r *Repository) ListNIS2OpenIncidents(ctx context.Context, orgID string) ([]NIS2IncidentRow, error) {
	rows, err := r.db.Query(ctx,
		`SELECT id::text, org_id::text, title, status,
		        nis2_early_warning_due, nis2_full_report_due, nis2_final_report_due,
		        nis2_early_warning_submitted_at, nis2_full_report_submitted_at, nis2_final_report_submitted_at
