...
func (h *Handler) ListBSIObjectDependencies(c echo.Context) error {
	id := c.Param("id")
	deps, err := h.service.ListBSIObjectDependencies(c.Request().Context(), orgID(c), id)
	if err != nil {
...
func (h *Handler) CreateBSIObjectDependency(c echo.Context) error {
	sourceID := c.Param("id")
	var in CreateBSIObjectDependencyInput
	...
	dep, err := h.service.CreateBSIObjectDependency(c.Request().Context(), orgID(c), sourceID, in)
	if err != nil {
		if isNotFound(err) {
...
func (h *Handler) DeleteBSIObjectDependency(c echo.Context) error {
	depID := c.Param("depId")
	err := h.service.DeleteBSIObjectDependency(c.Request().Context(), orgID(c), depID)
	if err != nil {
		if isNotFound(err) {
...
func (h *Handler) UpdateBSIObjectProtectionOverride(c echo.Context) error {
	id := c.Param("id")
	var in UpdateBSIObjectProtectionOverrideInput
	if err := c.Bind(&in); err != nil {
		return errResp(c, http.StatusBadRequest, "invalid request body", "CK_BAD_REQUEST")
	}
	if err := h.validate.Struct(in); err != nil {
	...
	obj, err := h.service.UpdateBSIObjectProtectionOverride(c.Request().Context(), orgID(c), id, in)
	if err != nil {
		if isNotFound(err) {
			return errResp(c, http.StatusNotFound, "target object not found", "CK_BSI_TO_NOT_FOUND")
		}
