...
func (h *Handler) ListBSITargetObjects(c echo.Context) error {
	objects, err := h.service.ListBSITargetObjects(c.Request().Context(), orgID(c))
	if err != nil {
...
func (h *Handler) CreateBSITargetObject(c echo.Context) error {
	var in CreateBSITargetObjectInput
	...
	obj, err := h.service.CreateBSITargetObject(c.Request().Context(), orgID(c), in)
	if err != nil {
...
func (h *Handler) GetBSITargetObject(c echo.Context) error {
	id := c.Param("id")
	obj, err := h.service.GetBSITargetObject(c.Request().Context(), orgID(c), id)
	if err != nil {
		if isNotFound(err) {
...
func (h *Handler) UpdateBSITargetObject(c echo.Context) error {
	id := c.Param("id")
	var in UpdateBSITargetObjectInput
	if err := c.Bind(&in); err != nil {
		return errResp(c, http.StatusBadRequest, "invalid request body", "CK_BAD_REQUEST")
	}
	if err := h.validate.Struct(in); err != nil {
	...
	obj, err := h.service.UpdateBSITargetObject(c.Request().Context(), orgID(c), id, in)
	if err != nil {
		if isNotFound(err) {
			return errResp(c, http.StatusNotFound, "target object not found", "CK_BSI_TO_NOT_FOUND")
		}
...
func (h *Handler) DeleteBSITargetObject(c echo.Context) error {
	id := c.Param("id")
	if err := h.service.DeleteBSITargetObject(c.Request().Context(), orgID(c), id); err != nil {
		if isNotFound(err) {
			return errResp(c, http.StatusNotFound, "target object not found", "CK_BSI_TO_NOT_FOUND")
		}
...
func (h *Handler) AssignBausteinToTargetObject(c echo.Context) error {
	...
	if err := h.service.AssignBausteinToTargetObject(c.Request().Context(), orgID(c), id, in.BausteinID); err != nil {
		if isNotFound(err) {
			return errResp(c, http.StatusNotFound, "target object not found", "CK_BSI_TO_NOT_FOUND")
		}
...
func (h *Handler) RemoveBausteinFromTargetObject(c echo.Context) error {
	...
	if err := h.service.RemoveBausteinFromTargetObject(c.Request().Context(), orgID(c), id, bausteinID); err != nil {
...
func (h *Handler) GetBSICheckSheet(c echo.Context) error {
	id := c.Param("id")
	results, err := h.service.GetCheckSheet(c.Request().Context(), orgID(c), id)
	if err != nil {
...
func (h *Handler) SetBSICheckResult(c echo.Context) error {
	...
	result, err := h.service.SetCheckResult(c.Request().Context(), orgID(c), id, anforderungID, in)
	if err != nil {
...
func (h *Handler) BulkSetBSICheckResults(c echo.Context) error {
	id := c.Param("id")
	var items []BulkCheckResultItem
	if err := c.Bind(&items); err != nil {
		return errResp(c, http.StatusBadRequest, "invalid request body", "CK_BAD_REQUEST")
	}
	if err := h.service.BulkSetCheckResults(c.Request().Context(), orgID(c), id, items); err != nil {
		if strings.Contains(err.Error(), "begruendung_required") {
...
func (h *Handler) GetBSICheckSummary(c echo.Context) error {
	id := c.Param("id")
	summary, err := h.service.GetCheckSummary(c.Request().Context(), orgID(c), id)
	if err != nil {
...
func (h *Handler) GetBSICockpit(c echo.Context) error {
	cockpit, err := h.service.GetBSICockpit(c.Request().Context(), orgID(c))
	if err != nil {
...
func (h *Handler) GetBSIGapReport(c echo.Context) error {
	report, err := h.service.GetBSIGapReport(c.Request().Context(), orgID(c))
	if err != nil {
...
func buildGapReportCSV(r BSIGapReport) string {
