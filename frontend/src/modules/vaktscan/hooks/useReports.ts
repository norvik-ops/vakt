...
export function useRiskTrend() {
  return useQuery<RiskTrendResponse>({
    queryKey: ['vaktscan', 'reports', 'risk-trend'],
    queryFn: () => apiFetch<RiskTrendResponse>('/vaktscan/reports/risk-trend'),
    staleTime: 30_000,
  })
}
...
export function useReports() {
  return useQuery<Report[]>({
    queryKey: ['vaktscan', 'reports'],
    queryFn: () => apiFetch<Report[]>('/vaktscan/reports'),
    staleTime: 5_000,
...
export function useDownloadReport() {
  return (reportId: string, title?: string) => {
    void fetch(`/api/v1/vaktscan/reports/${reportId}/download`, {
      credentials: 'include',
    })
      .then((r) => r.blob())
      .then((blob) => {
...
export function useCreateReport() {
  ...
  return useMutation<Report, Error, CreateReportInput>({
    mutationFn: (data) =>
      apiFetch<Report>('/vaktscan/reports', {
        method: 'POST',
        body: JSON.stringify({ title: data.title }),
      }),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['vaktscan', 'reports'] })
    },
  })
}
