...
export default function ChecklistRunPage() {
  ...
  const {
    ...
    queryKey: ['vakthr', 'checklist-runs', id],
    queryFn: () => apiFetch<ChecklistRun>(`/vakthr/checklist-runs/${id ?? ''}`),
    enabled: !!id,
    staleTime: 30 * 1000,
  })
  ...
  const {
    ...
    queryKey: ['vakthr', 'checklists', run?.checklist_id],
    queryFn: () => apiFetch<Checklist>(`/vakthr/checklists/${run?.checklist_id ?? ''}`),
  ...
  const {
    ...
    queryKey: ['vakthr', 'employees', run?.employee_id],
    queryFn: () => apiFetch<Employee>(`/vakthr/employees/${run?.employee_id ?? ''}`),
  ...
  const updateRun = useMutation<ChecklistRun, Error, { completed_items: string[]; status: ChecklistRun['status'] }>({
    mutationFn: ({ completed_items, status }) =>
      apiFetch<ChecklistRun>(`/vakthr/checklist-runs/${id ?? ''}`, {
        method: 'PUT',
