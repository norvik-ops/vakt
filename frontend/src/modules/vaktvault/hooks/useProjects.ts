...
export function useProject(id: string) {
  return useQuery<Project>({
    queryKey: ['vaktvault', 'projects', id],
...
export function useProjectHealth(projectId: string) {
  return useQuery<ProjectHealth>({
    queryKey: ['vaktvault', 'projects', projectId, 'health'],
...
export function useCreateProject() {
  ...
  return useMutation<Project, Error, CreateProjectInput>({
    ...
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['vaktvault', 'projects'] })
    },
  })
}
