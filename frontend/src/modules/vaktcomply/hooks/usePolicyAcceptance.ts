...
export function useCampaigns(policyId: string) {
  return useQuery<PolicyAcceptanceCampaign[]>({
    queryKey: ['vaktcomply', 'policies', policyId, 'acceptance-campaigns'],
    queryFn: () => apiFetch<PolicyAcceptanceCampaign[]>(`/vaktcomply/policies/${policyId}/acceptance-campaigns`),
    enabled: !!policyId,
    staleTime: 60 * 1000,
  })
}
...
export function useCreateCampaign(policyId: string) {
  ...
  return useMutation<PolicyAcceptanceCampaign, Error, CreateCampaignInput>({
    mutationFn: (input) =>
      apiFetch<PolicyAcceptanceCampaign>(`/vaktcomply/policies/${policyId}/acceptance-campaigns`, {
        method: 'POST',
...
export function useCampaignStats(campaignId: string) {
  return useQuery<CampaignStats>({
    queryKey: ['vaktcomply', 'acceptance-campaigns', campaignId, 'stats'],
    queryFn: () => apiFetch<CampaignStats>(`/vaktcomply/policies/acceptance-campaigns/${campaignId}/stats`),
    enabled: !!campaignId,
    staleTime: 30 * 1000,
  })
}
...
export function useCampaignRequests(campaignId: string) {
  return useQuery<PolicyAcceptanceRequest[]>({
    queryKey: ['vaktcomply', 'acceptance-campaigns', campaignId, 'requests'],
    queryFn: () =>
      apiFetch<PolicyAcceptanceRequest[]>(`/vaktcomply/policies/acceptance-campaigns/${campaignId}/requests`),
    enabled: !!campaignId,
    staleTime: 30 * 1000,
  })
}
