...
export function useControlExceptions(controlId: string) {
  return useQuery<ControlException[]>({
    queryKey: ['vaktcomply', 'exceptions', controlId],
    queryFn: () =>
      apiFetch<ControlException[]>(`/vaktcomply/exceptions?control_id=${controlId}`),
    enabled: !!controlId,
    staleTime: 5 * 60 * 1000,
  })
}
...
export function useCreateControlException(controlId: string) {
  ...
  return useMutation<ControlException, Error, CreateControlExceptionInput>({
    mutationFn: (input) =>
      apiFetch<ControlException>(`/vaktcomply/controls/${controlId}/exceptions`, {
        method: 'POST',
...
export function useUpdateControlException() {
  ...
  return useMutation<ControlException, Error, { id: string; input: UpdateControlExceptionInput }>({
    mutationFn: ({ id, input }) =>
      apiFetch<ControlException>(`/vaktcomply/exceptions/${id}`, {
        method: 'PUT',
        body: JSON.stringify(input),
      }),
    onSuccess: (data) => {
      void queryClient.invalidateQueries({
        queryKey: ['vaktcomply', 'exceptions', data.control_id],
      })
    },
  })
}
...
export function useDeleteControlException() {
  ...
  return useMutation<undefined, Error, { id: string; controlId: string }>({
    mutationFn: ({ id }) =>
      apiFetch<undefined>(`/vaktcomply/exceptions/${id}`, { method: 'DELETE' }),
