...
export function useCAPAs(statusFilter?: string, page = 1, limit = 25) {
  ...
  const query = useQuery<PaginatedResponse<CAPA>>({
    queryKey: ['vaktcomply', 'capas', statusFilter ?? 'all', page, limit],
    queryFn: () => apiFetch<PaginatedResponse<CAPA>>(`/vaktcomply/capas?${params.toString()}`),
    staleTime: 2 * 60 * 1000,
  })
  return {
    ...query,
...
export function useCAPAsForSource(sourceType: string, sourceId: string) {
  const path = sourceType === 'audit'
    ? `/vaktcomply/audits/${sourceId}/capas`
    : `/vaktcomply/incidents/${sourceId}/capas`
  return useQuery<CAPA[]>({
    queryKey: ['vaktcomply', 'capas', sourceType, sourceId],
...
export function useCreateCAPA() {
  ...
  return useMutation<CAPA, Error, CreateCAPAInput>({
    mutationFn: (input) =>
      apiFetch<CAPA>('/vaktcomply/capas', { method: 'POST', body: JSON.stringify(input) }),
    onSuccess: (data) => {
      void queryClient.invalidateQueries({ queryKey: ['vaktcomply', 'capas'] })
      if (data.source_id) {
        void queryClient.invalidateQueries({ queryKey: ['vaktcomply', 'capas', data.source_type, data.source_id] })
      }
    },
  })
}
...
export function useUpdateCAPA() {
  ...
  return useMutation<CAPA, Error, { id: string; input: UpdateCAPAInput }>({
    mutationFn: ({ id, input }) =>
      apiFetch<CAPA>(`/vaktcomply/capas/${id}`, { method: 'PATCH', body: JSON.stringify(input) }),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['vaktcomply', 'capas'] })
    },
  })
...
export function useDeleteCAPA() {
  ...
  return useMutation<undefined, Error, string>({
    mutationFn: (id) =>
      apiFetch<undefined>(`/vaktcomply/capas/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['vaktcomply', 'capas'] })
    },
  })
