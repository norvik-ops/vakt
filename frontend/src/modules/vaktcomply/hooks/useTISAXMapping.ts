...
export function useTISAXISOMapping(frameworkId?: string) {
  ...
  return useQuery<MappingResult[]>({
    queryKey: ['vaktcomply', 'tisax-iso-mapping', frameworkId ?? 'default'],
    queryFn: () => apiFetch<MappingResult[]>(`/vaktcomply/frameworks/tisax/iso-mapping${params}`),
    staleTime: 5 * 60 * 1000,
  })
}
...
export function useTISAXGapsAfterISO(frameworkId?: string) {
  const params = frameworkId ? `?framework_id=${frameworkId}` : ''
  return useQuery<MappingResult[]>({
    queryKey: ['vaktcomply', 'tisax-coverage-after-iso', frameworkId ?? 'default'],
    queryFn: () => apiFetch<MappingResult[]>(`/vaktcomply/frameworks/tisax/coverage-after-iso${params}`),
    staleTime: 5 * 60 * 1000,
  })
}
