...
function ReassessmentWizard({ runId, onComplete, onCancel }: WizardProps) {
  ...
  const submitAnswer = async (value: number) => {
    ...
    setSubmitting(true)
    try {
      await apiFetch(`/vaktcomply/reassess/${runId}/answer`, {
        method: 'POST',
...
export default function NIS2ReassessmentPage() {
  ...
  const {
    ...
    queryFn: () => apiFetch<HistoryResponse>('/vaktcomply/history'),
  })

  const startMutation = useMutation({
    mutationFn: () =>
      apiFetch<{ run_id: string }>('/vaktcomply/reassess', { method: 'POST', body: '{}' }),
    onSuccess: (data) => {
      setCooldownMsg(null)
