...
function TISAXTile({ tisaxFrameworkId }: { tisaxFrameworkId: string }) {
  ...
  return (
    <Link
      to={`/vaktcomply/frameworks/${tisaxFrameworkId}/tisax`}
...
export default function SecVitalsOverviewPage() {
  ...
  async function downloadExecutiveSummary() {
    ...
    setExecError(null)
    try {
      const res = await fetch('/api/v1/vaktcomply/reports/executive-summary', {
        credentials: 'include',
      })
