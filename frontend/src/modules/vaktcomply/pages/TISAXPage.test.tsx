...
function renderTISAXPage(frameworkId = 'fw-tisax', protectionLevel?: string) {
  const initialPath = `/vaktcomply/frameworks/${frameworkId}/tisax${protectionLevel ? `?protection_level=${protectionLevel}` : ''}`
  const client = new QueryClient({ defaultOptions: { queries: { retry: false } } })
  return render(
    <QueryClientProvider client={client}>
      <MemoryRouter initialEntries={[initialPath]}>
        <Routes>
          <Route path="/vaktcomply/frameworks/:id/tisax" element={<TISAXPage />} />
        </Routes>
      </MemoryRouter>
    </QueryClientProvider>,
  )
...
describe('TISAXPage', () => {
  ...
  it('click on "Bereitschaftsbericht exportieren" triggers fetch to tisax-report-pdf endpoint', () => {
    ...
    expect(fetchSpy).toHaveBeenCalledWith(
      expect.stringContaining('/api/v1/vaktcomply/frameworks/fw-tisax/tisax-report-pdf'),
