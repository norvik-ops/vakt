...
function NotApplicableDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) onClose() }}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t('vaktcomply.controlDetailPage.naDialogTitle')}</DialogTitle>
        </DialogHeader>
        <div className="py-3 space-y-3">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="na-reason">
              {t('vaktcomply.controlDetailPage.naReason')} <span className="text-secondary">{t('vaktcomply.controlDetailPage.naReasonHint')}</span>
            </Label>
            <textarea
              id="na-reason"
              rows={3}
              ...
              placeholder={t('vaktcomply.controlDetailPage.naPlaceholder')}
            />
          </div>
        </div>
        <DialogFooter>
          ...
          <Button onClick={handleConfirm} disabled={updateControl.isPending}>
            {updateControl.isPending ? t('vaktcomply.controlDetailPage.naSaving') : t('vaktcomply.controlDetailPage.naConfirm')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
function GapExplainPanel({ controlId, controlTitle }: { controlId: string; controlTitle: string }) {
  ...
  const handleExplain = () => {
    void start({
      endpoint: `/vaktcomply/ai/controls/${controlId}/explain`,
      prompt: controlTitle,
    })
  }
...
export default function ControlDetailPage() {
  ...
  const STATUS_CONFIG: Record<StatusChoice, { icon: React.ReactNode; label: string; className: string }> = {
    missing:        { icon: <Circle className="w-3.5 h-3.5" />,       label: t('vaktcomply.controlDetailPage.statusMissing'),        className: 'text-red-500' },
    in_progress:    { icon: <Clock className="w-3.5 h-3.5" />,        label: t('vaktcomply.controlDetailPage.statusInProgress'),     className: 'text-yellow-600' },
    implemented:    { icon: <CheckCircle2 className="w-3.5 h-3.5" />, label: t('vaktcomply.controlDetailPage.statusImplemented'),    className: 'text-green-600' },
    not_applicable: { icon: <MinusCircle className="w-3.5 h-3.5" />,  label: t('vaktcomply.controlDetailPage.statusNotApplicable'), className: 'text-secondary' },
  }
  ...
  const evidenceTypeLabel: Record<Evidence['type'], string> = {
    manual: t('vaktcomply.controlDetailPage.evidenceTypeManualLabel'),
    automated: t('vaktcomply.controlDetailPage.evidenceTypeAutomatedLabel'),
    document: t('vaktcomply.controlDetailPage.evidenceTypeDocumentLabel'),
  }
  ...
  function handleStatusChange(value: string) {
    ...
    updateControl.mutate(
      ...
      {
        onSuccess: () => toast(t('vaktcomply.controlDetailPage.toastSaved'), 'success'),
        onError: (err) => toast(handleApiError(err), 'error'),
      },
    )
  }
  ...
  function handleSubmitApprovalRequest() {
    requestApproval.mutate(
      ...
      {
        onSuccess: () => {
          toast(t('vaktcomply.controlDetailPage.toastApprovalSubmitted'), 'success')
  ...
  function handleAdd(e: React.FormEvent) {
    ...
    if (type === 'document' && file) {
      ...
      uploadEvidence.mutate(fd, {
        onSuccess: () => { resetAddForm(); toast(t('vaktcomply.controlDetailPage.toastEvidenceUploaded'), 'success') },
        onError: (err) => toast(handleApiError(err), 'error'),
      })
    } else {
      addEvidence.mutate(
        ...
        {
          onSuccess: () => { resetAddForm(); toast(t('vaktcomply.controlDetailPage.toastEvidenceAdded'), 'success') },
          onError: (err) => toast(handleApiError(err), 'error'),
        },
      )
    }
  ...
  function handleReview(e: React.FormEvent) {
    ...
    review.mutate(
      { status: reviewStatus, notes: reviewNotes || undefined },
      {
        onSuccess: () => {
          setReviewOpen(false)
          toast(t('vaktcomply.controlDetailPage.toastReviewDone'), 'success')
        },
        onError: (err) => toast(handleApiError(err), 'error'),
      },
    )
  ...
  const backTo = frameworkId
    ? `/vaktcomply/frameworks/${frameworkId}`
  ...
  return (
    <div className="flex flex-col h-full">
      <Breadcrumbs items={[
        ...
        { label: framework?.name ?? 'Framework', href: frameworkId ? `/vaktcomply/frameworks/${frameworkId}` : '/vaktcomply/frameworks' },
        ...
        actions={
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={exportControl}>
              ...
              {t('vaktcomply.controlDetailPage.export')}
            </Button>
            <Button size="sm" onClick={() => { setAddOpen(true); }}>
              <Plus className="w-4 h-4 mr-1" />
              {t('vaktcomply.controlDetailPage.addEvidence')}
            </Button>
            <Button variant="outline" size="sm" onClick={() => { navigate(backTo); }}>
              <ArrowLeft className="w-4 h-4 mr-1" />
              {t('vaktcomply.controlDetailPage.back')}
            </Button>
          </div>
        }
      />
      ...
      <div className="flex-1 p-6 space-y-6">
          ...
          <div className="p-5 bg-surface border border-border rounded-lg space-y-4">
            ...
            {control.not_applicable && control.not_applicable_reason && (
              <p className="text-xs text-secondary italic border-l-2 border-border pl-3">
                {t('vaktcomply.controlDetailPage.reasonLabel')}: {control.not_applicable_reason}
              </p>
            )}
            ...
            {control.description ? (
              ...
              <p className="text-sm text-secondary italic">{t('vaktcomply.controlDetailPage.noDescription')}</p>
            )}
        ...
        {subControls.length > 0 && (
          <Card>
            <CardHeader>
              <CardTitle className="text-sm">{t('vaktcomply.controlDetailPage.subControls')}</CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>{t('vaktcomply.controlDetailPage.colControlId')}</TableHead>
                    <TableHead>{t('vaktcomply.controlDetailPage.colTitle')}</TableHead>
                    <TableHead>{t('vaktcomply.controlDetailPage.colStatus')}</TableHead>
                    <TableHead></TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {subControls.map((sub) => {
                    ...
                    return (
                      <TableRow
                        ...
                        onClick={() =>
                          { navigate(
                            `/vaktcomply/controls/${sub.id}${frameworkId ? `?frameworkId=${frameworkId}` : ''}`,
                          ); }
                        }
                      >
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm flex items-center gap-2">
              ...
              {t('vaktcomply.controlDetailPage.implementationSteps')}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {/* Task list */}
                ...
                {tasks.map((task) => (
                  <li key={task.id} className="flex items-start gap-3 group">
                    <button
                      ...
                      title={task.completed ? t('vaktcomply.controlDetailPage.markOpen') : t('vaktcomply.controlDetailPage.markDone')}
                    >
                    ...
                    <button
                      ...
                      title={t('vaktcomply.controlDetailPage.deleteStep')}
                    >
            ...
            <form onSubmit={handleAddTask} className="flex gap-2">
              <Input
                placeholder={t('vaktcomply.controlDetailPage.addStep')}
                value={newTaskText}
            ...
            {(!tasks || tasks.length === 0) && !newTaskText && (
              <p className="text-xs text-muted-foreground">
                {t('vaktcomply.controlDetailPage.stepsEmpty')}
              </p>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent className="p-0">
            {evidenceLoading ? (
              ...
              <div className="flex flex-col items-center py-12 text-center">
                ...
                <p className="text-sm font-medium text-secondary">{t('vaktcomply.controlDetailPage.noEvidence')}</p>
                <p className="text-xs text-secondary mt-1">{t('vaktcomply.controlDetailPage.noEvidenceDesc')}</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>{t('vaktcomply.controlDetailPage.evidenceColTitle')}</TableHead>
                    <TableHead>{t('vaktcomply.controlDetailPage.evidenceColType')}</TableHead>
                    <TableHead>{t('vaktcomply.controlDetailPage.evidenceColStatus')}</TableHead>
                    <TableHead>{t('vaktcomply.controlDetailPage.evidenceColExpiry')}</TableHead>
                    <TableHead>{t('vaktcomply.controlDetailPage.evidenceColAdded')}</TableHead>
                    <TableHead></TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          {ev.status === 'pending_review' && (
                            ...
                            >
                              {t('vaktcomply.controlDetailPage.review')}
                            </Button>
                          )}
                          <Button
                            variant="ghost"
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">{t('vaktcomply.controlDetailPage.attachments')}</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <Dialog open={approvalDialogOpen} onOpenChange={(v) => { if (!v) { setApprovalDialogOpen(false); setApprovalComment(''); setPendingStatus('') } }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('vaktcomply.controlDetailPage.approvalDialogTitle')}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <p className="text-sm text-secondary">
              {t('vaktcomply.controlDetailPage.approvalDesc')}
            </p>
            <div className="text-sm space-y-1">
              <p>
                <span className="font-medium text-primary">{t('vaktcomply.controlDetailPage.requestedStatus')}:</span>{' '}
                <span className="text-brand font-medium">
                  {pendingStatus === 'missing' ? t('vaktcomply.controlDetailPage.statusMissing')
                    : pendingStatus === 'in_progress' ? t('vaktcomply.controlDetailPage.statusInProgress')
                    : pendingStatus === 'implemented' ? t('vaktcomply.controlDetailPage.statusImplemented')
                    : pendingStatus === 'not_applicable' ? t('vaktcomply.controlDetailPage.statusNotApplicable')
                    : pendingStatus}
                </span>
              </p>
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">{t('vaktcomply.controlDetailPage.approvalComment')}</Label>
              <Textarea
                value={approvalComment}
                onChange={(e) => { setApprovalComment(e.target.value); }}
                placeholder={t('vaktcomply.controlDetailPage.approvalPlaceholder')}
                rows={3}
              />
            </div>
          </div>
          <DialogFooter>
            ...
            <Button
              ...
              disabled={requestApproval.isPending}
            >
              {requestApproval.isPending ? t('vaktcomply.controlDetailPage.submitting') : t('vaktcomply.controlDetailPage.submitApproval')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{t('vaktcomply.controlDetailPage.addEvidenceTitle')}</DialogTitle></DialogHeader>
          <form onSubmit={(e) => { handleAdd(e) }}>
            <div className="py-4 space-y-4">
              <div className="space-y-1.5">
                <Label htmlFor="ev-title">{t('vaktcomply.controlDetailPage.evidenceLabelTitle')}</Label>
                <Input
                  id="ev-title"
                  value={title}
                  onChange={(e) => { setTitle(e.target.value); }}
                  placeholder={t('vaktcomply.controlDetailPage.evidencePlaceholderTitle')}
                  required
                />
              </div>
              <div className="space-y-1.5">
                <Label>{t('vaktcomply.controlDetailPage.evidenceLabelType')}</Label>
                <Select value={type} onValueChange={(v) => { setType(v as Evidence['type']); setFile(null) }}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="manual">{t('vaktcomply.controlDetailPage.evidenceTypeManual')}</SelectItem>
                    <SelectItem value="document">{t('vaktcomply.controlDetailPage.evidenceTypeDocument')}</SelectItem>
                    <SelectItem value="automated">{t('vaktcomply.controlDetailPage.evidenceTypeAutomated')}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              {type === 'document' && (
                <div className="space-y-1.5">
                  <Label htmlFor="ev-file">{t('vaktcomply.controlDetailPage.evidenceLabelFile')}</Label>
                  <input
                    id="ev-file"
                    type="file"
              ...
              {type === 'automated' && (
                <div className="flex items-start gap-2 p-3 bg-surface2 rounded-lg text-xs text-secondary">
                  ...
                  <div>
                    <p className="font-medium text-primary mb-1">{t('vaktcomply.controlDetailPage.automatedEvidenceTitle')}</p>
                    <p>{t('vaktcomply.controlDetailPage.automatedEvidenceDesc')}</p>
                    <Button
                      type="button"
                      size="sm"
                      variant="outline"
                    ...
                    >
                      ...
                      {collectEvidence.isPending ? t('vaktcomply.controlDetailPage.collecting') : t('vaktcomply.controlDetailPage.collectNow')}
                    </Button>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-1.5">
                <Label htmlFor="ev-notes">{t('vaktcomply.controlDetailPage.evidenceLabelNotes')}</Label>
                <textarea
                  id="ev-notes"
                  rows={3}
                  className="w-full rounded-md border border-border bg-surface2 text-primary px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-brand"
                  ...
                  placeholder={t('vaktcomply.controlDetailPage.evidencePlaceholderNotes')}
                />
              </div>

              <div className="space-y-1.5">
                <Label htmlFor="ev-expires-at">{t('vaktcomply.controlDetailPage.evidenceLabelExpiry')}</Label>
                <Input
                  id="ev-expires-at"
                  type="date"
                  value={expiresAt}
                ...
                <p className="text-xs text-secondary">
                  {t('vaktcomply.controlDetailPage.evidenceExpiryHint')}
                </p>
              </div>
            </div>
            <DialogFooter>
              ...
              >
                {addEvidence.isPending || uploadEvidence.isPending ? t('vaktcomply.controlDetailPage.adding') : t('vaktcomply.controlDetailPage.addEvidenceSubmit')}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
      ...
      <Dialog open={reviewOpen} onOpenChange={setReviewOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{t('vaktcomply.controlDetailPage.reviewTitle')}</DialogTitle></DialogHeader>
          <form onSubmit={(e) => { handleReview(e) }}>
            <div className="py-4 space-y-4">
              <div className="space-y-1.5">
                <Label>{t('vaktcomply.controlDetailPage.reviewDecision')}</Label>
                <Select value={reviewStatus} onValueChange={(v) => { setReviewStatus(v as 'approved' | 'rejected'); }}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="approved">{t('vaktcomply.controlDetailPage.reviewApprove')}</SelectItem>
                    <SelectItem value="rejected">{t('vaktcomply.controlDetailPage.reviewReject')}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="review-notes">{t('vaktcomply.controlDetailPage.reviewLabelNotes')}</Label>
                <textarea
                  id="review-notes"
                  rows={3}
                  className="w-full rounded-md border border-border bg-surface2 text-primary px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-brand"
                  ...
                  placeholder={t('vaktcomply.controlDetailPage.reviewPlaceholderNotes')}
                />
              </div>
            </div>
            <DialogFooter>
              ...
              <Button type="submit" disabled={review.isPending}>
                {review.isPending ? t('vaktcomply.controlDetailPage.reviewSaving') : t('vaktcomply.controlDetailPage.reviewComplete')}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
