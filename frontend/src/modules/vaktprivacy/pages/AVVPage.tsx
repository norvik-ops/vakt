...
function AVVCard({
  ...
  const STATUS_LABELS: Record<AVV['status'], string> = {
    active: t('vaktprivacy.avvPage.statusActive'),
    expired: t('vaktprivacy.avvPage.statusExpired'),
    terminated: t('vaktprivacy.avvPage.statusTerminated'),
  }
    ...
    <Card>
      <CardContent className="pt-5 space-y-2">
        ...
        <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
          {contractDate && <span>{t('vaktprivacy.avvPage.contractDate')}: {contractDate}</span>}
          {reviewDate && <span>{t('vaktprivacy.avvPage.reviewDate')}: {reviewDate}</span>}
        </div>
        {avv.template_id && (
          <p className="text-xs text-primary/70">{t('vaktprivacy.avvPage.template')}: {avv.template_id}</p>
        )}
  ...
  return (
    <div className="space-y-4 py-2">
      <div className="space-y-1.5">
        <Label>{t('vaktprivacy.avvPage.labelProcessor')} *</Label>
        <Input
          placeholder={t('vaktprivacy.avvPage.placeholderProcessor')}
          value={form.processor_name}
          onChange={(e) => { set({ processor_name: e.target.value }); }}
        />
      </div>
      <div className="space-y-1.5">
        <Label>{t('vaktprivacy.avvPage.labelService')} *</Label>
        <Textarea
          placeholder={t('vaktprivacy.avvPage.placeholderService')}
          rows={3}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1.5">
          <Label>{t('vaktprivacy.avvPage.labelContractDate')}</Label>
          <Input
            type="date"
        ...
        <div className="space-y-1.5">
          <Label>{t('vaktprivacy.avvPage.labelReviewDate')}</Label>
          <Input
            type="date"
      ...
      {showStatus && (
        <div className="space-y-1.5">
          <Label>{t('vaktprivacy.avvPage.labelStatus')}</Label>
          <Select value={form.status} onValueChange={(v) => { set({ status: v as AVV['status'] }); }}>
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="active">{t('vaktprivacy.avvPage.statusActive')}</SelectItem>
              <SelectItem value="expired">{t('vaktprivacy.avvPage.statusExpired')}</SelectItem>
              <SelectItem value="terminated">{t('vaktprivacy.avvPage.statusTerminated')}</SelectItem>
            </SelectContent>
          </Select>
        </div>
      )}
      <div className="space-y-1.5">
        <Label>{t('vaktprivacy.avvPage.labelNotes')}</Label>
        <Textarea
          placeholder={t('vaktprivacy.avvPage.placeholderNotes')}
          rows={2}
          value={form.notes}
...
export default function AVVPage() {
  ...
  return (
    <div className="flex flex-col h-full">
      <PageHeader
        title={t('vaktprivacy.avvPage.title')}
        description={t('vaktprivacy.avvPage.description')}
        tier="pro"
        actions={
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => { setTemplatePickerOpen(true); }}>
              <LayoutTemplate className="w-4 h-4 mr-1" />
              {t('vaktprivacy.avvPage.fromTemplate')}
            </Button>
            <Button onClick={openCreate}>
              <Plus className="w-4 h-4 mr-1" />
              {t('vaktprivacy.avvPage.createAVV')}
            </Button>
          </div>
        }
      />
      ...
      <div className="flex-1 p-6">
        ...
        {isError && (
          <div className="text-sm text-red-400 p-4 bg-red-500/10 rounded-lg">
            {t('vaktprivacy.avvPage.loadError')}
          </div>
        )}
        ...
        {!isLoading && !isError && avvs?.length === 0 && (
          <EmptyState
            icon={Handshake}
            title={t('vaktprivacy.avvPage.noAVVs')}
            description={t('vaktprivacy.avvPage.noAVVsDesc')}
            action={
      ...
      <AlertDialog open={deleteId !== null} onOpenChange={(open) => { if (!open) setDeleteId(null) }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('vaktprivacy.avvPage.deleteDialogTitle')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('vaktprivacy.avvPage.deleteDialogDesc')}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <Dialog open={dialogMode !== null} onOpenChange={(open) => { if (!open) setDialogMode(null) }}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {dialogMode === 'create' ? t('vaktprivacy.avvPage.createDialogTitle') : t('vaktprivacy.avvPage.editDialogTitle')}
            </DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSubmit} disabled={!canSubmit}>
              {isPending ? t('vaktprivacy.avvPage.saving') : dialogMode === 'create' ? t('vaktprivacy.avvPage.createAVV') : t('common.save')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
