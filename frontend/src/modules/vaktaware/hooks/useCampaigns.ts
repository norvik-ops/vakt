...
export function useCampaign(id: string) {
  return useQuery<Campaign>({
    queryKey: ['vaktaware', 'campaigns', id],
...
export function useCampaignStats(id: string) {
  return useQuery<CampaignStats>({
    queryKey: ['vaktaware', 'campaigns', id, 'stats'],
...
export function useCreateCampaign() {
  ...
  return useMutation<Campaign, Error, CreateCampaignInput>({
    ...
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['vaktaware', 'campaigns'] })
    },
  })
}
...
export function useLaunchCampaign(id: string) {
  ...
  return useMutation<{ status: string }>({
    ...
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['vaktaware', 'campaigns', id] })
      void queryClient.invalidateQueries({ queryKey: ['vaktaware', 'campaigns'] })
    },
  })
}
...
export function useDownloadCampaignReport() {
  return (campaignId: string, campaignName?: string) => {
    void fetch(`/api/v1/vaktaware/campaigns/${campaignId}/report`, {
      credentials: 'include',
    })
      .then((r) => r.blob())
      .then((blob) => {
