...
- **Doku-Konsistenz-Guard** — `scripts/check-docs.py` (+ `Docs`-Workflow `.github/workflows/docs.yml`, GitHub-hosted Runner) prüft bei jedem Doku-/`go.mod`/`config.go`-Push: Go-Version in den Stack-Docs == `backend/go.mod`, AI-Default-Modell == `config.go`, keine kaputten internen `.md`-Links, und jede Config-Variable aus `.env.example` **und** `config.go` ist in `docs/wiki/configuration.md` dokumentiert (Ops-Vars via `ENV_DOC_EXEMPT`). Quelle der Wahrheit ist immer der Code. Deckte 5 bis dahin undokumentierte Config-Vars auf (EPSS, AI-Cost/Cache/Limits, License-Refresh, Metrics, Sentry) — jetzt dokumentiert.
