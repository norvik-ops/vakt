...
Eine Fehlerklasse zieht sich durch dieses Release: **Funktionen, die Erfolg meldeten, ohne etwas getan zu haben.** Ein Offboarding, das den Zugang nicht entzog. Ein Löschantrag, der nichts löschte. Ein Audit-Paket, das bei jeder Kontrolle „fehlt" behauptete. Fristen-Mails, die nie hinausgingen, während das Protokoll die Zustellung bestätigte. Wer eine dieser Meldungen als Nachweis geführt hat, sollte den betroffenen Vorgang nach dem Update wiederholen — die betroffenen Stellen sind unten einzeln benannt.

**Minor-Release, kein Patch.** Der Enterprise-Tarif ist entfallen (siehe „Entfernt"), der Wertebereich für Schweregrade hat einen neuen Wert bekommen, das Abmelden widerruft mehr als bisher, und eine abgelaufene Pro-Lizenz verhält sich anders als zuvor. Bestehende Daten bleiben lesbar; alle 14 Migrationen laufen beim Start automatisch.

### Security

- **Fristen-Warnungen gehen nicht mehr verloren, wenn kein Mailserver eingerichtet ist.** Auf einer Instanz ohne SMTP meldete der Versand Erfolg, ohne etwas zu senden — und die Warnung wurde anschließend dauerhaft als „zugestellt" vermerkt. Sie ging danach **auch dann nicht mehr hinaus, als ein Mailserver eingerichtet wurde**. Betroffen waren die 72-Stunden-Meldefrist einer Datenpanne (Art. 33 DSGVO), überfällige Betroffenenanfragen, ablaufende AVV und gescheiterte Compliance-Prüfungen. Nicht Gesendetes wird jetzt nicht mehr vermerkt; die Meldung bleibt offen und geht hinaus, sobald ein Mailserver da ist. **Wer die Plattform ohne SMTP betrieben hat, sollte offene Datenpannen und Betroffenenanfragen einmal durchsehen** — für Vorgänge, die in dieser Zeit als benachrichtigt vermerkt wurden, ist keine Mail hinausgegangen. Bereits vermerkte Einträge werden **nicht** rückwirkend zurückgesetzt: ob eine Mail damals ankam, lässt sich im Nachhinein nicht feststellen, und eine Vermutung wäre schlechter als der ehrliche Hinweis.
- **Einladungsmails, die nicht zugestellt werden konnten, sagen es jetzt.** Ein Fehlschlag wurde vollständig verschluckt; die Einladung stand in der Liste und wirkte unterwegs. Der Einladungslink wird dabei bewusst **nicht** protokolliert — er ist ein Zugangsgeheimnis; er steht in der Einladungsliste und kann von dort geteilt werden.
- **Sicherheitsaktualisierung des Web-Frameworks** (`echo` v4.15.3, CVE-2026-55677): eine Abweichung beim Dekodieren von URL-Pfaden konnte Informationen preisgeben. Dazu eine Aktualisierung einer Testabhängigkeit (CVE-2026-17106).

- **Das Offboarding entzieht die Zugänge jetzt wirklich.** Ein vollständig abgearbeiteter Offboarding-Lauf — einschließlich des Pflichtschritts „Alle IT-Zugänge widerrufen (AD, VPN, SaaS)" — erzeugte einen Compliance-Nachweis im Status „freigegeben", während das Plattform-Konto derselben Person unverändert nutzbar blieb: aktive Sitzungen, gültige API-Schlüssel, bestehende Mitgliedschaft. Der Entzug findet jetzt tatsächlich statt (Sitzungen, API-Schlüssel, Mitgliedschaft, Mitarbeiterstatus); scheitert er, entsteht **kein** Nachweis und der Lauf bleibt offen. **Wer Offboarding-Nachweise aus einer früheren Version als Beleg führt, sollte für die betroffenen Personen prüfen, ob das Konto noch besteht.**
- **Ein Löschantrag nach Art. 17 DSGVO lässt sich nicht mehr als erledigt stempeln, ohne dass gelöscht wurde.** Der „Erledigen"-Knopf wurde auch für Löschanträge angeboten und setzte den Status unmittelbar auf „abgeschlossen", ohne die Löschung auszuführen: abgeschlossener Antrag, null gelöschte Zeilen — und das Verzeichnis-PDF druckte „Löschung (Art. 17) — Erledigt". Als vollzogen gilt jetzt nur, was die Löschung selbst innerhalb ihrer Transaktion vermerkt hat; ein Teil-Erfolg ist ausgeschlossen, und die Regel gilt auf beiden Schreibwegen. **Löschanträge, die eine frühere Version als erledigt geführt hat, sind erneut zu prüfen.**
- **Kein Einblick mehr in die Richtlinien-Daten fremder Organisationen.** Zwei Endpunkte der Richtlinien-Akzeptanz — Kampagnen-Kennzahlen und Empfängerliste — prüften ausschließlich die Kampagnen-Kennung und an keiner Stelle die Organisation. Wer eine fremde Kennung kannte, las die Kennzahlen sowie Namen und E-Mail-Adressen der Empfänger einer anderen Organisation, mit einem Viewer-Konto und in identischer Form wie der Eigentümer.
- **Ein Administrator kann keine Kennwort-Zurücksetzung mehr für ein Konto außerhalb seiner Organisation auslösen.** Der Endpunkt suchte das Zielkonto allein über die plattformweit eindeutige E-Mail-Adresse und gab den Rücksetz-Link samt Klartext-Token in der Antwort zurück — der Administrator einer beliebigen Organisation konnte damit das Konto eines fremden Administrators übernehmen. Die Suche ist jetzt auf die eigene Organisation begrenzt, und das Token verlässt den Server über diese Antwort nicht mehr.
- **Auditor-, Lieferanten- und Betroffenenanfrage-Links stehen nicht mehr im Klartext im Zugriffsprotokoll.** Diese Token liegen in der Datenbank bewusst nur als Hash, damit ein Log- oder Sicherungs-Leak wertlos ist. Sie stehen jedoch in einem Pfadsegment der Adresse, und die Maskierung deckte nur den Abfrageteil ab. Wer die Protokolle lesen konnte, konnte die Links innerhalb ihrer Gültigkeit wiederverwenden.
- **Drei fehlende Rechteprüfungen sind nachgezogen.** Ein Viewer konnte den KI-Agenten starten, über den NIS2-Assistenten den Umsetzungsstand **aller** Kontrollen der Organisation überschreiben und den Ungelesen-Status der Benachrichtigungen für sämtliche Mitglieder zurücksetzen. Für den letzten Fall trug die Ausnahmeliste der Rechteprüfung eine Begründung, die nie zutraf.
- **Ein Schlüsseltausch macht die Passwortverwaltung nicht mehr unlesbar.** Der Rotationslauf übersprang stillschweigend jedes dort abgelegte Geheimnis und meldete sich trotzdem als erfolgreich — nach dem Tausch wären sämtliche Vault-Secrets dauerhaft verloren gewesen. Sechs weitere verschlüsselte Felder (SSO-Geheimnis, SMTP- und LDAP-Zugangsdaten, Sicherungs-Passphrase) hatten überhaupt keine Rotationsstufe. Der Lauf rechnet jetzt gegen die tatsächliche Zahl verschlüsselter Spalten, benennt unlesbare Zeilen und schlägt fehl, statt zu überspringen (ADR-0084, Migration 266). **Wer vor diesem Update einen Schlüsseltausch durchgeführt hat, muss die betroffenen Geheimnisse als verloren behandeln und neu hinterlegen.**
- **Eine Redis-Verbindung über `rediss://` ist im Hintergrunddienst jetzt tatsächlich verschlüsselt.** Die TLS-Einstellung ging beim Aufbau der Verbindung verloren — sie sah verschlüsselt aus und war es nicht. Betroffen ist jede Installation, die Redis über ein nicht vertrauenswürdiges Netz erreicht. Der Benutzername für Redis-Zugriffslisten kam ebenfalls nie an.
- **Ein maskiertes Geheimnis wird nicht mehr als echtes Geheimnis gespeichert.** Ließ sich der bestehende Wert einer Cloud-Anbindung nicht laden, verschlüsselte das Speichern den Maskierungs-Platzhalter und meldete die Anbindung als konfiguriert — sie hätte sich mit der Zeichenfolge `****` angemeldet. Das Speichern verlangt jetzt eine erneute Eingabe.
- **Ein fehlgeschlagener Widerruf beim Abmelden wird als Fehler gemeldet.** Bisher antwortete das Abmelden auch dann mit Erfolg, wenn beide Sperrpfade scheiterten; das Token blieb bis zu seinem Ablauf gültig.
...
- **Das Audit-Paket weist wieder den tatsächlichen Umsetzungsstand aus.** Für jede Organisation stand in der `controls.csv` bei jeder einzelnen Kontrolle „missing", und die Lückenanalyse zeigte einen Gesamtwert von 0 % — beide lasen eine Spalte, die kein Programmteil je beschrieben hat. Es ist genau die Datei, die die beiliegende `README.txt` dem Auditor zum Ausdrucken empfiehlt. Der Umsetzungsstand wird jetzt in der Datenbank aus dem gepflegten Status abgeleitet (ADR-0082, Migration 254); an einer echten Instanz gemessen liefert dieselbe Datenlage nun 35/50/57 % statt 0 %. **Audit-Pakete aus einer früheren Version sind unbrauchbar und neu zu erzeugen.**
- **Der Scanner-Import funktioniert.** Der Import von SARIF-, CycloneDX-, CSV- und Wazuh-Ergebnissen scheiterte für **jeden** Aufrufer an einem Datenbankfehler — seit es die Funktion gibt, ohne dass je ein Import gelungen wäre. Ein zweiter Lauf mit denselben Daten aktualisiert die vorhandenen Funde jetzt, statt sie zu verdoppeln.
- **Eine abgelaufene Pro-Lizenz sperrt Sie nicht mehr aus Ihren eigenen Daten aus.** Zugesichert war, dass eine abgelaufene Pro-Lizenz den Lesezugriff behält und lediglich Schreibvorgänge sperrt. Im Code gab es zwei Prüfungen mit unterschiedlicher Auffassung davon, und 164 der 167 Zugänge benutzten die falsche: Sie beantworteten auch reine Leseabrufe mit „Zahlung erforderlich". Es gilt jetzt überall die zugesicherte Regel.
- **Hochladen und Importieren ist wieder möglich.** Acht angemeldete Schreibvorgänge — Nachweis-Upload, Nachweis-Dateien, Lieferanten-CSV-Import, KI-Bericht, Fund-Import, Anlagen-Import (beide Wege) und der NIS2-PDF-Export — wies der Server mit „403" ab, weil sie den Anfrage-Token nicht mitschickten. Sie waren vom Tag ihrer Einführung an unbenutzbar, nicht bloß ungeschützt.
- **Das Bestellformular funktioniert im Browser.** Der Rechnungsdienst beantwortete die Vorabanfrage des Browsers ohne die nötige Freigabe für die Herkunft der Website, also brach der Browser die Bestellung ab und zeigte „Das hat nicht geklappt." Kein Kunde konnte auf diesem Weg bestellen.
- **Fristen- und Alarm-Mails werden zugestellt, statt nur protokolliert zu werden.** Drei unabhängige Ursachen im selben Weg: Der Hintergrunddienst hatte den Mailversand an zwölf Stellen gar nicht verdrahtet und schrieb trotzdem „Benachrichtigung versandt"; drei Alarm-Aufgaben setzten ihre 24-Stunden-Sperre, bevor überhaupt etwas zugestellt war (bei dauerhaftem Zustellproblem also für immer stumm, ADR-0083); und die 12-Stunden-Vorwarnung setzte ihre Marke auch bei fehlgeschlagenem Versand, sodass sie sich nie wiederholte. Betroffen waren NIS2- und DORA-Meldefristen, SLA-Fristen, ablaufende Auftragsverarbeitungsverträge und überfällige Betroffenenanfragen. Zusätzlich war der Zustell-Auftrag für Benachrichtigungen im Hintergrunddienst nicht registriert.
- **Die Anwendbarkeitserklärung (SoA) hat wieder eine Wahrheit.** Drei Ausgabewege — PDF, CSV und der Export nach PDF/XLSX/DOCX — lasen drei verschiedene Quellen und widersprachen einander im selben Moment: Das PDF wies eine Kontrolle als „Anwendbar: Nein" samt Ausschlussbegründung aus, die CSV im selben Augenblick als „Ja". Für ISO 27001 Klausel 6.1.3 ist dieses Dokument der Nachweis. Die zweite Spalte wird jetzt aus der ersten abgeleitet (Migration 264, ADR-0082); bei widersprüchlichen Altdaten setzt sich der **Ausschluss** durch. **Prüfen Sie Ihre Anwendbarkeitserklärung nach dem Update einmal durch.**
- **Scan-Ergebnisse fließen wieder als Nachweis nach Vakt Comply.** Die Brücke zwischen Vakt Scan und Vakt Comply war korrekt gebaut, bekam aber nie ein Ereignis: Der einzige auslösende Programmteil hatte keinen einzigen Aufrufer, während alle neun tatsächlichen Wege — Trivy, Nuclei, OpenVAS und die vier Importe — an ihm vorbeiliefen. Aus demselben Grund hat der ausgehende Webhook `finding.created` nie ausgelöst, obwohl er in der Oberfläche als abonnierbares Ereignis angeboten wurde.
- **Ein einzelner Fund mit unbekanntem Schweregrad lässt keinen Scan mehr fehlschlagen.** Trivy und Nuclei liefern regulär den Schweregrad „unknown"; die Datenbank lehnte ihn ab, und weil alle Funde eines Laufs gemeinsam geschrieben werden, riss diese eine Zeile den gesamten Scan mit. Der Scan galt danach als fehlgeschlagen (Migration 265, siehe auch „Geändert").
- **Funde aus SARIF-Dateien werden wieder zusammengeführt und priorisiert.** Der SARIF-Import verwarf die CVE-Kennung, obwohl sie in der Datei steht. Damit erkannte die scanner-übergreifende Zusammenführung nichts als Dublette — zwei Scanner auf derselben Anlage ergaben vier Einträge statt zwei — und die Risikoanreicherung übersprang diese Funde ausdrücklich, weil ihnen die CVE fehlte. Beides sind Eigenschaften, mit denen Vakt Scan ausdrücklich beworben wird.
- **Die automatische Schulungszuweisung schreibt wieder.** Weder das Onboarding einer neuen Person noch die Nachschulung nach einem Phishing-Klick hat je eine Zuweisung erzeugt: Der eine Auslöser war nirgends angemeldet, der andere schrieb Werte, die die Datenbank ablehnte, und beide Fehler wurden verschluckt. Folge über die reine Funktion hinaus: Der BSI-Bericht wies ORP.3.A2/A3/A5 als erfüllt aus, weil eine Regel *existierte*, nicht weil sie je gewirkt hätte — und dieser Bericht fließt als Nachweis nach Vakt Comply.
- **Die formale Risikoakzeptanz ist erreichbar.** Der vollständig verdrahtete Freigabedialog antwortete ausnahmslos mit einem Konflikt, weil die Vorbedingung einen Wert verlangte, den weder die Eingabeprüfung noch die Datenbank je zugelassen haben — es gab keinen Weg, sie herzustellen, auch nicht von Hand. Die Prüfung liest jetzt den Registerstatus, in dem die Entscheidung tatsächlich liegt.
- **Die NIS2-Meldepflicht ist wieder feststellbar, und die Fristen laufen ab dem richtigen Zeitpunkt.** Die drei Kriterien der Meldepflicht kamen wegen einer abweichenden Datenform nie beim Server an — die Kennzeichnung „Meldepflichtig" erschien deshalb nie. Zugleich rechnete die Frist ab dem Zeitpunkt der *Bewertung* statt ab der *Entdeckung*: Ein vor drei Tagen entdeckter Vorfall bekam eine 24-Stunden-Frist ab jetzt angezeigt, obwohl sie gesetzlich zwei Tage zuvor abgelaufen war. Der Anker kommt jetzt aus dem hinterlegten Entdeckungszeitpunkt. Die Frist für den Abschlussbericht rechnete „einen Monat" als 30 Tage und ist auf einen Kalendermonat umgestellt.
- **Vier Cloud-Anbindungen sind nach dem Speichern wieder lesbar.** Prometheus, Keycloak, GitLab und SonarQube schrieben ihre Konfiguration in einer Form, die der eigene Lesevorgang nicht verarbeiten konnte: Die Einstellungsseite und der Abrufdienst bekamen nichts zurück. Beim GitLab-Zugang ging dabei zusätzlich der echte Zugriffstoken still verloren, sobald man erneut speicherte. Ebenfalls behoben: Die Freigabe für Ziele im eigenen Netz wurde von keiner Lesefunktion zurückgelesen — der Schutz vor Anfragen ins interne Netz lehnte damit genau die Ziele ab, für die die Freigabe gebaut worden war.
- **Der DSGVO-Datenexport ist wieder auswertbar.** Jede Kennung — und damit jede Beziehung zwischen den 22 Dateien des Archivs — stand als Liste von 16 Zahlen statt als lesbare Kennung. Kein übliches Werkzeug kann daraus die Verknüpfungen herstellen, obwohl das Archiv ausdrücklich als Nachweis der Datenübertragbarkeit nach Art. 20 DSGVO angeboten wird. Der Formatwechsel ist in der Datei `meta.json` als Version ausgewiesen; dort stand außerdem für jede Vakt-Version die Angabe „dev".
- **API und Hintergrunddienst arbeiten wieder auf derselben Redis-Datenbank.** Trug die Verbindungsadresse eine Datenbanknummer, legte die API ihre Aufträge in Datenbank 0 ab, während der Hintergrunddienst aus Datenbank N las: Jede Auftragskette brach ohne Fehler, ohne Protokolleintrag und ohne Wiederholung ab. Die mitgelieferte Compose-Konfiguration nutzt keine Nummer und war deshalb nicht betroffen.
- **Die Kennzahlen der Auftragswarteschlange zeigen auch Wiederholungen und Archiviertes.** Die Warteschlangentiefe zählte nur wartende und laufende Aufträge — bei acht wartenden Wiederholungen und zwanzig archivierten Fehlschlägen meldeten alle sieben Warteschlangen weiterhin 0. Der Fehlerzähler verlor seinen Wert zudem nach sieben Tagen und traf damit ausgerechnet die seltenen Fehler.
- **Der automatische Sicherungsweg erzeugt wieder Sicherungen.** Der beworbene Weg produzierte null Backups; vier Ursachen wirkten zusammen, von der Netzwerkzuordnung bis zu einem ungeprüften Rückgabewert, der eine korrekte Wiederherstellung als Totalausfall behandelte. Die Mindestgrößenprüfung, die eine leere Sicherung verhindern sollte, war selbst wirkungslos — ein Archiv mit leerem Datenbank-Abzug kam über die Schwelle. Der Rückweg (Wiederherstellung) trug denselben Fehler und ist mitbehoben.
- **PDF-Berichte zeigen wieder Umlaute, und ein fehlgeschlagener Export lädt keine leere Datei mehr herunter.** Die Berichte wurden mit einer Zeichensatz-Einstellung erzeugt, die deutsche Sonderzeichen verstümmelt. Schlug die Erzeugung fehl, bekam man dennoch eine Datei mit dem Status „OK" — nur leer. Kennzahlen mit Nachkommastellen wurden beim Speichern zudem stillschweigend verworfen.
- **Seiten stürzen im Leerzustand nicht mehr ab.** Mehrere Listen-Endpunkte antworteten auf einen leeren Bestand mit „nichts" statt mit einer leeren Liste, woran die zugehörigen Seiten abstürzten — betroffen war also genau der Zustand jeder neuen Installation. Ebenfalls behoben: Das Auditor-Portal stürzte ab, weil eine Antwort eine andere Form hatte als die Schnittstellenbeschreibung zusagte.
- **Die Freigabe des ISMS-Geltungsbereichs und der Managementbewertung ist wieder möglich.** Beide Freigabewege antworteten **jedem** mit „keine Berechtigung", weil eine zweite, überflüssige Prüfung einen Wert las, den nirgends jemand setzte. Der ISO-27001-Freigabeweg war damit still ausgefallen. Es gilt jetzt allein die Prüfung an der Route (Geltungsbereich: Administrator, Managementbewertung: interner Auditor).
- **Mehrere Schreibvorgänge melden keinen Erfolg mehr, ohne einen Datensatz zu treffen.** Betroffen waren vier Vorgänge in Vakt Aware (unter anderem das Zustellergebnis einer Kampagne und die Unzustellbarkeits-Markierung), der Nachweis am Ende eines Offboarding-Laufs und die Cloud-Synchronisierung, die null Nachweise erhob und den Lauf trotzdem als „erfolgreich" führte.
- **Eine Störung meldet Sie nicht mehr ab.** War der Server kurzzeitig überlastet oder die Anfragezahl begrenzt, behandelte die Oberfläche das wie eine ungültige Sitzung und meldete gültig angemeldete Nutzer ab.

### Changed

- **Schweregrade kennen einen neuen Wert: „unbewertet".** Trivy und Nuclei liefern regulär Funde ohne Bewertung. Sie werden nicht mehr auf „Info" abgebildet, denn „Info" behauptet „geprüft und harmlos" — ein unbewerteter Fund ist etwas anderes. In der Sortierung steht „unbewertet" zwischen „kritisch" und „hoch", damit er nicht am unteren Ende der Liste verschwindet. Filter, gespeicherte Ansichten und Auswertungen, die alle Schweregrade einzeln aufzählen, sollten diesen Wert aufnehmen (Migration 265).
- **Das Abmelden beendet jetzt auch die langlebige Sitzung.** Bisher lief der Widerruf ins Leere: Die 30-Tage-Sitzung überlebte das Abmelden, und die Sperre bereits ausgegebener Token wurde nie hochgezählt. Beides greift jetzt. Praktische Folge: Ein Abmelden beendet den Zugang wirklich — auch für einen Token, den jemand außerhalb des Browsers aufbewahrt hat.
- **Bei widersprüchlichen Angaben zur Anwendbarkeit einer Kontrolle gewinnt der Ausschluss.** Beim Zusammenführen der bisher zwei getrennten Spalten kann eine Kontrolle in Altdaten gleichzeitig als anwendbar und als ausgeschlossen geführt sein. Da „anwendbar" der Vorgabewert beider Spalten war, ist der Ausschluss die tatsächlich getroffene Entscheidung und setzt sich durch.
- **Ein Lizenzschlüssel mit unbekannter Tarifstufe schaltet nichts frei.** Ein gültig signierter Schlüssel mit einer Stufe, die nicht mehr verkauft wird, fällt auf die Community-Edition zurück, statt sein Merkmalsverzeichnis in Kraft zu setzen. Kein Kunde ist betroffen — kein Ausstellungsweg hat je eine andere Stufe als „Pro" vergeben.

### Removed

- **Der Enterprise-Tarif ist entfallen.** Er war über keinen Weg ausstellbar: Beide Signierpfade vergaben ausschließlich „Pro", und das Verwaltungswerkzeug kannte keine Tarifoption. Die Preisfläche bewarb darüber dennoch drei Rahmenwerke — TISAX, DORA und ISO 42001 —, deren 25 Zugänge jedem zahlenden Kunden „Zahlung erforderlich" antworteten. Ein Tarif, den niemand kaufen kann, ist kein Tarif. Die drei Rahmenwerke sind jetzt ausdrücklich **nicht im Angebot** und im Katalog auch so beschriftet, statt als „Entwurf" — VDA ISA 6.0, das seit Januar 2025 geltende DORA und ISO/IEC 42001:2023 sind keine Entwürfe. Es gibt weiterhin genau zwei Tarife: Community und Pro.
- **Sieben Merkmale sind sichtbar teurer geworden — sie waren fälschlich als kostenlos beworben.** DSGVO Art. 32 (TOM), CIS Controls v8, KRITIS-DachG, BSI C5:2020, ISO 27017, ISO 27018 und der Audit-PDF-Export standen in den Produkttexten als Bestandteil der Community-Edition. Im Code hingen sie seit jeher an der Pro-Stufe; wer sie in der Community-Edition aufrief, bekam „Zahlung erforderlich". Geändert hat sich nichts am Verhalten der Software — nur die Texte sagen jetzt, was tatsächlich gilt. Wer die Preisseite kannte, wird den Unterschied bemerken; wir halten es für die ehrlichere Fassung.
- **Fünf Merkmale sind umgekehrt sichtbar günstiger geworden.** Ausgehende Webhooks, die Nachweis-Historie, das Continuous Control Monitoring, die AVV-Verwaltung und die LDAP-Synchronisierung wurden als Pro-Merkmale ausgewiesen, tragen im Code aber keine Tarifprüfung. Sie sind in der Community-Edition enthalten und waren es die ganze Zeit.
- **Zwei nie gebaute Merkmale sind aus den Verkaufstexten gestrichen.** „Eigene Controls anlegen" — es gibt keinen Weg dafür — sowie der „erweiterte PDF-Export mit eigenem Branding" samt der MSP-Dokumentation, die Logo und Fußzeile in Berichten versprach. Ebenfalls richtiggestellt: Lizenzschlüssel sind **nicht** an die Domain der Instanz gebunden; wer deshalb zusätzliche Schlüssel bestellt hat, brauchte sie nicht.
- **Eine veraltete Preisseite ist gelöscht.** Ein nicht mehr ausgeliefertes Vorgängerverzeichnis enthielt eine Preisseite, die sich dreifach selbst widersprach, einen falschen Jahrespreis nannte und BSI-Grundschutz sowie den EU AI Act der Community-Edition zuschlug. Die aktuelle Website war davon nicht betroffen.
