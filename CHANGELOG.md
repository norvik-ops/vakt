...
- **`scripts/build-lead-magnet-pdf.mjs`** — erzeugt ein gebrandetes PDF aus `docs/marketing/lead-magnets/*.md` (Playwright). Das PDF ist eingecheckt, nicht CI-gebaut (Playwright im Sites-Build wäre unverhältnismäßig für ein Asset, das sich selten ändert). Das Skript filtert interne Redaktionsnotizen aus der Quell-Markdown, damit sie nicht ins Kunden-PDF geraten.
